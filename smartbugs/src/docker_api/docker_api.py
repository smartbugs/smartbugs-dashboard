...
def analyse_files(tool, file, logs, now):
    try:
        ...
        results_folder = 'smartbugs/results/' + tool + '/' + now
        ...
        if 'default' not in cfg['docker_image'] or cfg['docker_image'] == None:
            logs.write(
                tool + ': default docker image not provided. please check you config file.\n')
            sys.exit(
                tool + ': default docker image not provided. please check you config file.')
        ...
        try:
            ...
            parse_results(output, tool, file_name, container,
                          cfg, logs, results_folder, start, end)

            with open(os.path.join('smartbugs/results.json'), 'w') as f:
                try:
